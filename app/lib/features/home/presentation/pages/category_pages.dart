...
class CategoryPages extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CategoryPages> createState() => _SingleCategoryState();
}

class _SingleCategoryState extends ConsumerState<CategoryPages> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      final state = ref.read(filterdBooksControllerProvider(widget.categoryId));
  ...
  Widget _buildBooksSection(BuildContext context) {
    final bookState = ref.watch(
      filterdBooksControllerProvider(widget.categoryId),
    );
        ...
        slivers: [
          ...
          SliverAppBar(
            ...
            flexibleSpace: Stack(
              ...
              children: [
                Positioned.fill(
                  child: _buildHeroBackground(widget.coverUrl ?? ""),
                ),
              ],
            ),
          ),
...
class Typography extends StatelessWidget {
  final String title;
  const Typography({required this.title});
